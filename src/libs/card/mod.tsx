...
export function FlipCard(props: { type: string } & { icon: ReactNode } & { color: Nullable<string> } & { title: Nullable<string> } & { subtitle?: Nullable<string> } & { index?: Nullable<number> } & { flip: boolean } & { onFlipChange(flip: boolean): void }) {
  ...
  return <button className="@container w-[min(20rem,100%)] aspect-video perspective-midrange text-start cursor-pointer hover:scale-105 focus-visible:outline-none focus-visible:scale-105 transition-transform"
    type="button"
    onClick={onClick}>
