...
export function MoneroAccountPage(props: { $entry: KDBX.Inner.KeePassFile.Entry }) {
  const { $entry } = props
  ...
  const sigkey = useMemo(() => {
    return $entry.getDirectStringByKeyOrNull("MoneroPrivateSpendingKey")?.getValueOrThrow().get()
  }, [$entry])
  ...
  return <div className="flex flex-col grow p-6">
    ...
    <form className="grow flex flex-col">
      ...
      {sigkey && <Fragment>
        ...
        <div className="text-default-contrast">
          Your Monero private spending key
        </div>
        <div className="h-4" />
        <div className="bg-default-contrast po-2 rounded-xl flex items-center gap-4 [&:has(:focus-visible)]:outline-2 [&:has(:focus-visible)]:outline-offset-2 [&:has(:focus-visible)]:outline-default-contrast">
...
export function MoneroAccountAddMenuAnchor() {
  const path = usePathContext().getOrThrow()
  const hash = useHashSubpath(path)

  const coords = useAnchorWithCoords(hash, "/monero")
...
export function StandaloneMoneroAccountAddPage() {
  const path = usePathContext().getOrThrow()
  const hash = useHashSubpath(path)
  ...
  const [$sigskey, setSigSpendKey] = useState("")
  ...
  const sigskey = useDeferredValue($sigskey)
  ...
  const getAddressOrNull = useCallback(async () => {
    if (!sigskey)
      return
    if (!/^(0x)?[0-9a-fA-F]{64}$/.test(sigskey))
      return

    const sigskeyAsHex = sigskey.startsWith("0x") ? sigskey.slice(2) : sigskey

    const sigskeyAsRaw = Uint8Array.fromHex(sigskeyAsHex)
    const sigskeyAsNum = BigInt("0x" + sigskeyAsRaw.toReversed().toHex())
  ...
  const encryptOrThrow = useCallback(async () => {
    ...
    if (address)
      $entry.createStringOrThrow("MoneroAddress", address)

    if (sigskey)
      $entry.createStringOrThrow("MoneroPrivateSpendingKey", sigskey, true)
  ...
  }, [session, title, color, address, sigskey, notes])
      ...
      <form className="grow flex flex-col">
        ...
        <div className="bg-default-contrast po-2 rounded-xl flex items-center gap-4 [&:has(:focus-visible)]:outline-2 [&:has(:focus-visible)]:outline-offset-2 [&:has(:focus-visible)]:outline-default-contrast">
          ...
          <input className="w-full focus-visible:outline-none"
            autoComplete="off"
            type={masked ? "password" : "text"}
            onChange={e => setSigSpendKey(e.target.value)}
            value={$sigskey} />
          <div className="flex items-center gap-2">
            <button className="group rounded-full p-1 hover:bg-default-double-contrast focus-visible:bg-default-double-contrast focus-visible:outline-none"
              type="button"
