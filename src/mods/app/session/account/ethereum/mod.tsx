...
export function EthereumAccountPage(props: { $entry: KDBX.Inner.KeePassFile.Entry }) {
  const { $entry } = props
  ...
  const sigkey = useMemo(() => {
    return $entry.getDirectStringByKeyOrNull("EthereumPrivateKey")?.getValueOrThrow().get()
  }, [$entry])
...
export function StandaloneEthereumAccountAddPage() {
  const path = usePathContext().getOrThrow()
  const hash = useHashSubpath(path)
  ...
  const getAddressOrNull = useCallback(async () => {
    if (!sigkey)
      return
    if (!/^(0x)?[0-9a-fA-F]{64}$/.test(sigkey))
      return

    const sigkeyAsHex = sigkey.startsWith("0x") ? sigkey.slice(2) : sigkey
    const sigkeyAsRaw = Uint8Array.fromHex(sigkeyAsHex)

    const pubkeyAsRaw = secp256k1.getPublicKey(sigkeyAsRaw, false)
    const digestAsRaw = keccak_256(pubkeyAsRaw.subarray(1))

    const addressAsHex = digestAsRaw.subarray(-20).toHex()
    const addressAsUtf = new TextEncoder().encode(addressAsHex)

    let address = "0x"

    const checksum = keccak_256(addressAsUtf).toHex()

    for (let i = 0; i < addressAsHex.length; i++)
      address += parseInt(checksum[i], 16) >= 8 ? addressAsHex[i].toUpperCase() : addressAsHex[i]
  ...
  const encryptOrThrow = useCallback(async () => {
    ...
    if (address)
      $entry.createStringOrThrow("EthereumAddress", address)

    if (sigkey)
      $entry.createStringOrThrow("EthereumPrivateKey", sigkey, true)
